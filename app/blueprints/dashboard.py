...
def create_event():
    ...
    new_event = Event(
        ...
        month=data.get(
            "month", event_date.strftime("%b").upper() if event_date else ""
        ),
...
@dashboard_bp.route("/api/courses", methods=["GET", "POST"])
def courses_api():
    """Get all courses or create a new course for current teacher."""
    user = get_current_user()
    if not user:
        return jsonify({"error": "Unauthorized"}), 401
    ...
    if request.method == "GET":
        ...
        new_course = Course(
            ...
            room=data.get("room", ""),
            icon_url=data.get("icon_url", "/static/images/course_default.png"),
            teacher_id=user.id,
        )
...
def schedule_view():
    ...
    if not user:
        return redirect(url_for("auth.login_page"))
    return render_template(
        "schedule.html", page="schedule", username=f"{user.first_name} {user.last_name}"
    )
